...
<head>
  ...
  <title>ShopEasy | Livreur - @yield('title')</title>
...
<body>
  ...
  <aside id="adminSidebar" class="admin-sidebar" role="navigation" aria-label="Barre latérale principale">
    ...
    <nav class="mt-2 d-flex flex-column" aria-label="Menu principal">
      <a href="{{ route('livreur.dashboard') }}" class="nav-link-admin {{ request()->routeIs('livreur.dashboard.*') ? 'active' : '' }}">
      ...
      <div class="dropdown">
        <a class="d-flex align-items-center text-decoration-none" id="profileMenu" data-bs-toggle="dropdown" aria-expanded="false" href="#">
          ...
          <div class="d-none d-md-block text-start">
            @if ($user->boutique?->nom)
              <div style="font-weight:700">{{ $user->boutique?->nom ?? 'Propriétaire' }}</div>
            @else
              <div style="font-weight:700">{{ auth()->user()->name ?? 'Propriétaire' }}</div>
            @endif
