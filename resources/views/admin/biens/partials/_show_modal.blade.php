    ...
    <div class="modal fade" id="bienModal{{ $bien->id }}" tabindex="-1" aria-hidden="true">
      ...
      <div class="modal-dialog modal-fullscreen-lg-down modal-dialog-centered">
        <div class="modal-content shadow-lg border-0 rounded-3">
          ...
          <!-- Header -->
          <div class="modal-header bg-dark text-warning fw-semibold">

            <h5 class="modal-title ">
              <img src="{{ asset('assets/images/logo_sahashop.png') }}" width="40px;height:40px;" alt="">
              Détails du Bien : {{ $bien->titre }}
            </h5>
          ...
          <div class="modal-body">
                    ...
                    <div class="carousel-item {{ $first ? 'active' : '' }}">
                      @if($media->type === 'image')
                            <img src="{{ asset('storage/' . $media->path) }}"
                                class="d-block w-100 rounded img-zoomable"
                                style="max-height:70vh; object-fit:contain; cursor: zoom-in;">

                      @elseif($media->type === 'video')
                      <video class="d-block w-100 rounded" 
                            style="max-height:70vh; object-fit:contain; cursor: pointer;" 
                            controls 
                            preload="metadata" 
                            playsinline>
            ...
            <div class="mt-4">
              ...
              <p class=""><strong>Catégorie: </strong>{{ $bien->categorie->name ?? '' }}</p>
              ...
              <p><strong>Propriétaire :</strong> {{ $bien->proprietaire->name }} {{ $bien->proprietaire->surname ?? '—' }}</p>
          ...
          document.querySelectorAll('.carousel').forEach(function (carousel) {
              ...
              carousel.addEventListener('slide.bs.carousel', function (e) {
                  ...
                  carousel.querySelectorAll('.carousel-item').forEach(function (item) {
                      ...
                      // Reset zoom
                      item.querySelectorAll('.img-zoomable').forEach(img => {
                          img.style.transform = 'scale(1)';
                      });
                  });
      
                  // Rendre visible uniquement le slide cible
                  if (e.relatedTarget) {
